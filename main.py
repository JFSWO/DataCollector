...
def transfer_execute_download(host, username, password, local_script_path,local_output_path, remote_script_path, log_file,command):
    ...
    try:
        ...
        stin, stdout, stderr = ssh.exec_command(f'unzip -o {remote_script_path_file} -d {remote_collection_dir}')
        ...
        stin, stdout, stderr = ssh.exec_command(full_command)
        stdout.channel.recv_exit_status()#wait for the command to finish
        log(log_file, f"Executing Command {full_command}")



    	# List files in the remote output directory to find the matching file
        ...
        if output_files:
            ...
            local_output_path_file = os.path.join(local_output_path, f'{host}_all.tar')
...
for cred in credentials:
    transfer_execute_download(cred['host'],cred['username'],cred['password'],cred['local_script_path'],cred['local_output_path'],cred['remote_script_path'],log_file,command)
